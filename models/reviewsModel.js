...
exports.removeReview = async (review_id, user_id) => {
  const queryStr = `DELETE FROM reviews WHERE review_id = $1 AND guest_id = $2 RETURNING *;`;

  const { rows } = await db.query(queryStr, [review_id, user_id]);

  if (rows.length === 0) {
    return Promise.reject({
      status: 404,
      msg: "Review not found or not authorized.",
    });
  }
